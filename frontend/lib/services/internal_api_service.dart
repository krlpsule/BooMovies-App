...
class InternalApiService {
  ...
  Future<void> addUser(String name, String username, String email, String password) async {
    final response = await http.post(
  ...
  Future<Map<String, dynamic>> addOrGetBook(Map<String, dynamic> bookData) async {
    final response = await http.post(
  ...
  Future<Map<String, dynamic>> addOrGetMovie(Map<String, dynamic> movieData) async {
    final response = await http.post(
  ...
  // --- YORUM EKLEME ---
  Future<void> postBookReview(int userId, int bookId, int rating, String reviewText) async {
    final response = await http.post(
  ...
  Future<void> postMovieReview(int userId, int movieId, int rating, String reviewText) async {
    final response = await http.post(
  ...
  Future<List<dynamic>> getUserBookReviews(int userId) async {
    final response = await http.get(Uri.parse('$baseUrl/user/$userId/book_reviews'));
    if (response.statusCode == 200) {
  ...
  Future<List<dynamic>> getUserMovieReviews(int userId) async {
    final response = await http.get(Uri.parse('$baseUrl/user/$userId/movie_reviews'));
    if (response.statusCode == 200) {
      return json.decode(response.body);
    }
    return [];
